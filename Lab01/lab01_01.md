...
1. 에이전트의 기본 설정이 완료되었습니다. 이제 에이전트의 이름을 정해야 합니다. "Contoso 고객 지원 에이전트"라는 이름을 제안드립니다. 이 이름이 괜찮으신가요?
   ```
   Yes
   ```

2. 또한 상담원이 도와주거나 이야기해서는 안 될 주제나 작업에 대한 질문도 받게 됩니다.
...
3. 상담원을 더욱 맞춤화하기 위해 대화형 환경으로 리디렉션됩니다. 설명에 대한 추가 세부 정보를 제공할 수 있으며(거부할 수 있음) 음성 톤도 묻습니다.
...
6. 메시지가 표시되면 **I'm done** 이라고 대답할 수 있습니다.

7. copilotD을 만들기 전에 "..." 및 "고급 설정 편집" 메뉴로 이동하여 이름과 성을 추가하여 에이전트 스키마 이름을 업데이트합니다</br>
